...
class ObjectRow(Base):
    ...
    __table_args__ = (
        UniqueConstraint("world_id", "scope", "id", name="uq_object_scope_id"),
    )
    ...
    definition_id: Mapped[str | None] = mapped_column(
        ForeignKey("object_definitions.id")
    )
...
class TransactionRow(Base):
    ...
    __table_args__ = (
        UniqueConstraint("world_id", "command_id", name="uq_world_command"),
    )
...
class ExecutionRow(Base):
    ...
    model_input: Mapped[list[dict[str, Any]]] = mapped_column(
...
def create_database(path: str | Path) -> tuple[Engine, sessionmaker]:
    ...
    url = (
        ...
        if database_path == ":memory:"
        else f"sqlite+pysqlite:///{database_path}"
    )
