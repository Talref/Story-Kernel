...
@app.command()
def run(
    database: Annotated[Path, typer.Option("--database", "-d", help="SQLite database path.")] = Path(
        ".story-kernel/story-kernel.db"
    ),
    ...
    open_browser: Annotated[bool, typer.Option("--open-browser", help="Open the browser on launch.")] = False,
) -> None:
    """Initialize persistence and launch the complete Gradio experiment harness."""
    harness = create_harness(database)
    build_interface(harness).launch(server_name=host, server_port=port, inbrowser=open_browser)
